...
`GET /tasks` can filter by `status` and `priority`. Task data is persisted as a JSON list in `app/data/tasks.json`. The app loads environment variables with `load_dotenv()` and reads `APP_ENV`, defaulting to `development`, for the API description.
...
`app/models_legacy.py` is an older model file and is not part of the current public model exports. No separate `app/storage.py` file is visible in the drafts; persistence is handled by `app/repositories/task_repository.py`.
...
- Calculate overdue filtering client-side.
